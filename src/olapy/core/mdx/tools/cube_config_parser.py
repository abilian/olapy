"""Parse cube configuration excel file and create cube parser object which can be
    ...
    return [
        {
            ...
            "columns": _get_columns(dimension),
        }
        for dimension in config["dimensions"]
    ]


def _get_facts(config):
    return {
...
def parse_cube_config(cube_config_file):
    """Parse cube config file. Result obj can ben passed to MdxEngine.
    (expected default file: ~/olapy-data/cube/cubes-config.yml)

    :param conf_file: full path to config file
    :return: cube obj
    """
    with open(cube_config_file, encoding="utf8") as config_file:
